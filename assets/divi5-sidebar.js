(function () {
	...
	function fieldByLabels(labels, selector) {
	...
	function fieldValueByLabels(labels, selector) {
		var match = fieldByLabels(labels, selector);
	...
	function fetchItemsForField(field, callback) {
		var ids = field.value || '';
		var gallery = ids ? '' : fieldValueByLabels(gallerySlugLabels, 'input');
		var nonce = restNonce();
	...
	function enhanceSidebarImageField(field) {
		...
		slugField = fieldByLabels(gallerySlugLabels, 'input');
		if (slugField) {
